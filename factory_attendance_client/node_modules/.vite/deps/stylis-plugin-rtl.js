...
import { COMMENT, DECLARATION, IMPORT, RULESET, compile, serialize, strlen } from "./stylis.js";
/*!
...
var import_cssjanus = /* @__PURE__ */ __toESM((/* @__PURE__ */ __commonJSMin(((exports, module) => {
	var cssjanus;
	/**
...
"transform": function(css, options) {
			...
			if (options.transformDirInUrl) css = css.replace(ltrDirSelector, "$1" + temporaryLtrToken + "$2").replace(rtlDirSelector, "$1" + temporaryRtlToken + "$2").replace(ltrInUrlRegExp, "$1" + temporaryToken).replace(rtlInUrlRegExp, "$1ltr").replace(temporaryTokenRegExp, "rtl").replace(temporaryLtrTokenRegExp, "ltr").replace(temporaryRtlTokenRegExp, "rtl");
			if (options.transformEdgeInUrl) css = css.replace(leftInUrlRegExp, "$1" + temporaryToken).replace(rightInUrlRegExp, "$1left").replace(temporaryTokenRegExp, "right");
...
//#endregion
export { stylisRTLPlugin as default };
